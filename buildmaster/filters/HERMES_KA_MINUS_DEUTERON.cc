...
void HERMES_KA_MINUS_DEUTERONFilter::ReadData()
{
  //Opening files
  fstream f1, f2;
  ...
  datafile << dataPath()
	   << "rawdata/HERMES/hermes.deuteron.zQ2-3D.zQ2-proj.vmsub.mults_kminus.list";
  ...
  covfile << dataPath()
	  << "rawdata/HERMES/hermes.deuteron.zQ2-3D.zQ2-proj.vmsub.covmat_mults.list";
  ...
  for(int i=0; i<fNData; i++)
      ...
      for(int j=0; j<fNData; j++)
	{
	  ...
	  getline(f2,line);
	  istringstream lstream(line);
	  lstream >> row >> col >> ddum >> ddum >> ddum >> covmat[i][j];
	  fSys[i][j].name = "CORR";
	  fSys[i][j].type = ADD;
	}
    }
  ...
  for(int i=0; i<fNData; i++)
      ...
      for(int j=0; j<fNData; j++)
	{
	  ...
	  cout << "    - {label: add, value: " << fSys[i][j].mult << "}" << endl;
	}
