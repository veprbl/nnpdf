...
f=open("./filtered_data_forward.dat", "a") 
data = []
stat = []
uncorr_sys = []
corr_sys = []
bins = [1,2,3,4]
for bin in bins:
    data_, stat_, uncorr_sys_, corr_sys_ = np.genfromtxt("./data_forward.dat", usecols=(bin*4+0,bin*4+1,bin*4+2,bin*4+3),
        delimiter='±', unpack=True, missing_values='-', skip_header=1)
    # the 1e3 factor is to convert from mb to microb    
